...
<div id="primary">
	...
	<?php if ( ! class_exists( 'Auto_Load_Next_Post' ) ) { ?>
		...
		<?php if ( ( get_theme_mod( 'colormag_author_bio_disable_setting', 0 ) == 0 ) && ( get_the_author_meta( 'description' ) ) ) : ?>
			<div class="author-box">
				...
				<div class="author-description-wrapper">
					...
					<?php if ( get_theme_mod( 'colormag_author_bio_links', 0 ) == 1 ) { ?>
						<p class="author-url"><?php printf( __( '%1$s has %2$s posts and counting.', 'colormag' ), get_the_author_meta( 'display_name' ), absint( count_user_posts( get_the_author_meta( 'ID' ) ) ) ); ?>
							<a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>"><?php printf( __( 'See all posts by %1$s', 'colormag' ), get_the_author_meta( 'display_name' ) ); ?></a>
						</p>
					<?php } ?>
				</div>
			</div>
