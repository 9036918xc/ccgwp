...
function accelerate_customize_register($wp_customize) {
	...
	class Accelerate_Important_Links extends WP_Customize_Control {
		...
		public function render_content() {
			...
			$important_links = array(
				...
				'documentation' => array(
					'link' => esc_url('https://themegrill.com/theme-instruction/accelerate-pro/'),
					'text' => __('Documentation', 'accelerate'),
				),
				'demo' => array(
					'link' => esc_url('https://demo.themegrill.com/accelerate-pro/'),
	...
	if ( ! function_exists( 'the_custom_logo' ) ) {
		...
		$wp_customize->add_setting('accelerate[accelerate_header_logo_image]', array(
			'default' => '',
			'type' => 'option',
			'capability' => 'edit_theme_options',
		...
		$wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'accelerate[accelerate_header_logo_image]', array(
			'label' => __('Upload logo for your header.', 'accelerate'),
			'section' => 'accelerate_header_logo',
			'setting' => 'accelerate[accelerate_header_logo_image]'
		)));
	}
	...
	if ( ! function_exists( 'wp_update_custom_css_post' ) ) {
		...
		$wp_customize->add_setting('accelerate[accelerate_custom_css]', array(
			'default' => '',
			'type' => 'option',
			'capability' => 'edit_theme_options',
		...
		$wp_customize->add_control(new Accelerate_Custom_CSS_Control($wp_customize, 'accelerate[accelerate_custom_css]', array(
			'label' => __('Write your custom css.', 'accelerate'),
			'section' => 'accelerate_custom_css_setting',
			'settings' => 'accelerate[accelerate_custom_css]'
		)));
	}
	...
	if ( ! function_exists( 'has_site_icon' ) ) {
		 ...
		 $wp_customize->add_setting('accelerate[accelerate_activate_favicon]', array(
			'default' => 0,
			'type' => 'option',
			'capability' => 'edit_theme_options',
			'sanitize_callback' => 'accelerate_checkbox_sanitize'
		 ...
		 $wp_customize->add_control('accelerate[accelerate_activate_favicon]', array(
		 	'type' => 'checkbox',
			...
			'settings' => 'accelerate[accelerate_activate_favicon]'
		 ));
		 ...
		 $wp_customize->add_setting('accelerate[accelerate_favicon]', array(
			'default' => 0,
			'type' => 'option',
			'capability' => 'edit_theme_options',
			'sanitize_callback' => 'esc_url_raw'
		 ...
		 $wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'accelerate[accelerate_favicon]', array(
			'label' => __('Upload favicon for your site.', 'accelerate'),
			'section' => 'accelerate_favicon_upload_section',
			'settings' => 'accelerate[accelerate_favicon]'
		 )));
	}
	...
	class Accelerate_Editor_Custom_Control extends WP_Customize_Control {
		...
		public $type = "accelerate_editor_control";
	...
	$default_footer_value = __( 'Copyright &copy; ', 'accelerate' ).'[the-year] [site-link] '.__( 'Theme by: ', 'accelerate' ).'[tg-link] '.__( 'Powered by: ', 'accelerate' ).'[wp-link]';
	...
	// Start of the Typography Options
	...
	foreach ($accelerate_fonts_families as $accelerate_fonts_family) {
		...
		// assigning the function for google font
		$accelerate_google_fonts_family = accelerate_google_fonts();

		$wp_customize->add_control($accelerate_fonts_family['id'], array(
			'label' => $accelerate_fonts_family['title'],
			'type' => 'select',
			'section' => 'accelerate_google_font_section',
			'setting' => $accelerate_fonts_family['id'],
			'choices' => $accelerate_google_fonts_family
		));
	...
	function accelerate_fonts_sanitize( $input, $setting ) {
		// Get the list of choices from the control associated with the setting.
		$choices = $setting->manager->get_control( $setting->id )->choices;
		// If the input is a valid key, return it, else, return the default.
		return ( array_key_exists( $input, $choices ) ? $input : $setting->default );
